...
Процедура ОбработкаОбслуживанияНачалоВыбора(Элемент
	...
	ОповещениеВыбораОбработкиОбслуживания = Новый ОписаниеОповещения("ОбработкаОбслуживанияНачалоВыбораОбработка", ктв_ОбработкиОбслуживанияКлиент, Параметры);
	
	ВремМеню = ктв_ОбработкиОбслуживанияВызовСервера.ПолучитьСписокОбработокОбслуживания(ПрефиксОбработки, ИмяКонтрольнойПроцедуры, ПараметрыКонтрольнойПроцедуры);
...
Процедура ОбработкаОбслуживанияНачалоВыбораОбработка(ВыбранныйЭлемент, Параметры) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = "ВыбратьИзФайла" Тогда
		ОповещениеВыбораФайлаОбработкиОбслуживания = Новый ОписаниеОповещения("ОбработкаОбслуживанияНачалоВыбораОбработкаВыбораФайла", ктв_ОбработкиОбслуживанияКлиент, Параметры);
...
Процедура ОбработкаОбслуживанияНачалоВыбораОбработкаВыбораФайла(Результат, Адрес, ВыбранноеИмяФайла, Параметры) Экспорт
	...
	Если НЕ ктв_ОбработкиОбслуживанияВызовСервера.ЭтоОбработкаОбслуживания(Адрес, ИмяКонтрольнойПроцедуры, ПараметрыКонтрольнойПроцедуры) Тогда
		СообщениеПользователю = Новый СообщениеПользователю();
		СообщениеПользователю.Текст = "Выбранный файл не является обработкой обслуживания!";
		СообщениеПользователю.ПутьКДанным = ПутьКДаннымОбработкаПредставление;
		СообщениеПользователю.Сообщить();
		Возврат;
	КонецЕсли;
	
	ОписаниеОбработки = ктв_ОбработкиОбслуживанияВызовСервера.ПолучитьОписаниеОбработкиОбслуживания(Адрес);
...
Процедура ОбработкаОбслуживанияОткрытие(Элемент
	...
	ОписаниеОбработки = ктв_ОбработкиОбслуживанияВызовСервера.ПолучитьОписаниеОбработкиОбслуживания(Форма[ПутьКДаннымОбработкаАдресВХранилище]);
	...
	Если ОписаниеОбработки.СписокДействийКнопкиОткрытия.Количество() = 1 Тогда
		ктв_ОбработкиОбслуживанияКлиент.ОбработкаОбслуживанияВыборДействияКнопкиОткрытия(ПервыйЭлемент, Параметры);
	Иначе
		ОповещениеВыбораДействияКнопкиОткрытия = Новый ОписаниеОповещения("ОбработкаОбслуживанияВыборДействияКнопкиОткрытия", ктв_ОбработкиОбслуживанияКлиент, Параметры);
...
Процедура ОбработкаОбслуживанияВыборДействияКнопкиОткрытия(ВыбранныйЭлемент, Параметры) Экспорт
	...
	Если Лев(ВыбранныйЭлемент.Значение, 13) = "ОткрытьФорму_" Тогда
		...
		ФормаОбработки = ктв_ОбработкиОбслуживанияКлиент.ПолучитьФормуОбработкиОбслуживания(Форма[ПутьКДаннымОбработкаАдресВХранилище], ОписаниеОбработки.ИмяОбработки, Сред(ВыбранныйЭлемент.Значение, 14), ПараметрыОткрытия, Форма);
	...
	ИначеЕсли Лев(ВыбранныйЭлемент.Значение, 17) = "ВыполнитьКоманду_" Тогда
		...
		ФормаОбработки = ктв_ОбработкиОбслуживанияКлиент.ПолучитьФормуОбработкиОбслуживания(Форма[ПутьКДаннымОбработкаАдресВХранилище], ОписаниеОбработки.ИмяОбработки, ОписаниеОбработки.ФормаОбработчиковКоманд, ПараметрыОткрытия, Форма);
...
Функция ПолучитьФормуОбработкиОбслуживания(АдресОбработки, ИмяОбработки = "", ИмяФормы = "", ПараметрыОткрытия = Неопределено, Владелец = Неопределено) Экспорт
	...
	ОписаниеОбработки = ктв_ОбработкиОбслуживанияВызовСервера.ПолучитьОписаниеОбработкиОбслуживания(АдресОбработки, ИмяОбработки);
